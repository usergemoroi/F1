...
const ImVec2 label_size = CalcTextSize(label, NULL, true);
const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(slider_width, label_size.y + 40));
const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + 20), total_bb.Max);
...
ImU32 redFill = ImColor(ImVec4(138 / 255.0f, 3 / 255.0f, 3 / 255.0f, 0.4f)); // alpha = 0.4
window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), redFill, 5.0f);

float center_y = (frame_bb.Min.y + frame_bb.Max.y) * 0.5f;
window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x - 1, center_y), 6.0f, ImColor(1.0f, 1.0f, 1.0f), 30);
